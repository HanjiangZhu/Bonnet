...
class SegRunner(BaseRunner):
    ...
    def run_model_trainval(self, batch_data):
        ...
        result = batch_data['bones'].clone()
        result.feats = logits
        result = result.to_flat_format()
        logits = result.feats
    ...
    def on_end_step(self, step_counter: int, batch_list: Dict[str, torch.Tensor]):
        if step_counter == 0 and self.epoch == 0 and self.cfg.use_wandb and self.split == "train":
            batch_data = batch_list[0]['bones'].to_dict('voxel')
            class_colors = np.asarray(self.cfg.data.class_colors)

            for b in range(min(3, batch_list[0]['batch_size'])):
                gt, hu = batch_list[0]['bones'].to_wandb(class_colors, b)
                wandb.log({
                    f"GT_({b},{self.epoch},{step_counter})": gt,
                    f"HU_({b},{self.epoch},{step_counter})": hu,
                })